...
insertPoll
  ...
  -> SqlPersistT
       Handler
       (Key Poll, [Key PollAvailableDate], Key PollUser)
...
getPollForm :: T.Text -> SqlPersistT Handler (Maybe PollForm)
getPollForm friendlyUrl =
  (getBy $ UniquePollUrl friendlyUrl) >>= (maybe (return Nothing) $ \(Entity pollId Poll {..}) -> do                                                                                
    applicableDays <-
